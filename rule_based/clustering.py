...
def search_phrases_in_json(json_content, phrases):
    """Search for phrases in JSON content (case-insensitive) using regex."""
...
def categorize_json_file(json_file_path, early_phrases, late_phrases, model=None, min_matches=2, semantic_threshold=0.6):
    ...
    try:
        ...
        features = {
            'early_exact_count': len(early_exact_matches),
            'early_semantic_count': len(early_semantic_matches),
            'late_exact_count': len(late_exact_matches),
            'late_semantic_count': len(late_semantic_matches),
            'early_total': len(early_matches),
            'late_total': len(late_matches),
        }
        ...
        if early_qualifies and late_qualifies:
            ...
            if len(early_matches) > len(late_matches):
                category = 'early'
                matches = early_matches
                match_details = {'exact': early_exact_matches, 'semantic': early_semantic_matches}
            elif len(late_matches) > len(early_matches):
                category = 'late'
                matches = late_matches
                match_details = {'exact': late_exact_matches, 'semantic': late_semantic_matches}
            else:
            ...
            category = 'unrelated'
            matches = []
            match_details = {'exact': [], 'semantic': []}
        
        return category, matches, match_details, features
...
def perform_advanced_clustering(features_list, file_names, n_clusters=3):
    """Perform advanced clustering using KMeans on feature vectors."""
    ...
    for features in features_list:
        feature_vector = [
            features.get('early_exact_count', 0),
            features.get('early_semantic_count', 0),
            features.get('late_exact_count', 0),
            features.get('late_semantic_count', 0),
            features.get('early_total', 0),
            features.get('late_total', 0)
        ]
    ...
    if len(feature_matrix) < n_clusters:
        print(f"Warning: Not enough files ({len(feature_matrix)}) for {n_clusters} clusters")
        return {}
    ...
    try:
        ...
        # Map file names to cluster labels
        file_to_cluster = {}
        for i, filename in enumerate(file_names):
            file_to_cluster[filename] = int(cluster_labels[i])
        ...
        for cluster_id in range(n_clusters):
            ...
            print(f"Cluster {cluster_id} ({len(cluster_indices)} files):")
            print(f"  Mean early exact: {mean_features[0]:.2f}")
            print(f"  Mean early semantic: {mean_features[1]:.2f}")
            print(f"  Mean late exact: {mean_features[2]:.2f}")
            print(f"  Mean late semantic: {mean_features[3]:.2f}")
            print(f"  Mean early total: {mean_features[4]:.2f}")
            print(f"  Mean late total: {mean_features[5]:.2f}")
            print()
...
def cluster_json_files(input_dir, output_base_dir, early_yml, late_yml, min_matches=2, use_semantic=True, semantic_threshold=0.6, use_advanced_clustering=False):
    """Cluster JSON files into early, late, or unrelated categories using exact and semantic matching."""
    ...
    # If using advanced clustering, create additional cluster directories
    if use_advanced_clustering:
        for i in range(5):  # Create up to 5 cluster directories
            cluster_dir = os.path.join(output_base_dir, f'cluster_{i}')
    ...
    # Collect features for advanced clustering
    features_list = []
    file_names = []
    file_categories = {}
    ...
    for json_file in json_files:
        ...
        category, matches, match_details, features = categorize_json_file(
        ...
        # Store for advanced clustering
        features_list.append(features)
        file_names.append(json_file)
        file_categories[json_file] = category
    ...
    # Perform advanced clustering if requested
    file_to_cluster = {}
    if use_advanced_clustering:
        print("=" * 50)
        print("PERFORMING ADVANCED CLUSTERING...")
        file_to_cluster = perform_advanced_clustering(features_list, file_names, n_clusters=3)
    
    # Copy files to appropriate directories
    for json_file in json_files:
        json_path = os.path.join(input_dir, json_file)
        category = file_categories[json_file]
        ...
        # Advanced clustering copy (if enabled)
        if use_advanced_clustering and json_file in file_to_cluster:
            cluster_id = file_to_cluster[json_file]
            cluster_dir = os.path.join(output_base_dir, f'cluster_{cluster_id}')
            cluster_dest_path = os.path.join(cluster_dir, json_file)
            shutil.copy2(json_path, cluster_dest_path)
    ...
    if use_advanced_clustering and file_to_cluster:
        print("\nAdvanced Clustering Results:")
        cluster_counts = {}
        for cluster_id in file_to_cluster.values():
    ...
    results_file = os.path.join(output_base_dir, 'clustering_results.json')
    results = {
        'summary': {
    ...
    if use_advanced_clustering:
        results['advanced_clustering'] = file_to_cluster
...
def main():
    ...
    output_base_dir = '/home/med_data/<user>/rule_based/clustered_output'
    ...
    use_advanced_clustering = True  # Enable advanced clustering with KMeans
    
    print(f"Clustering JSON files from: {input_dir}")
    print(f"Output directory: {output_base_dir}")
    print(f"Early phrases file: {early_yml}")
    print(f"Late phrases file: {late_yml}")
    print(f"Advanced clustering enabled: {use_advanced_clustering}")
    print()
